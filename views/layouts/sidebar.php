<aside class="left-sidebar bg-sidebar">
  <div id="sidebar" class="sidebar sidebar-with-footer">
    ...
    <?php
      ...
      $book = new Book();
      $total_books_uk = $book->where("book_origin","uk")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
      $count_total_uk = count($total_books_uk);
      $completed_books_uk = $book->where("status_id","12")->where("book_origin","uk")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
      $count_uk = count($completed_books_uk);
      $percentage_uk = round(($count_uk*100)/$count_total_uk,2);

      $book_usa = new Book();
      $total_books_usa = $book_usa->where("book_origin","usa")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
      $count_total_usa = count($total_books_usa);
      $completed_books_usa = $book_usa->where("status_id", "12")->where("book_origin","usa")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
      $count_usa = count($completed_books_usa);
      $percentage_usa = round(($count_usa*100)/$count_total_usa,2);

      $book_uae = new Book();
      $total_books_uae = $book_uae->where("book_origin","uae")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
      $count_total_uae = count($total_books_uae);
      $completed_books_uae = $book_uae->where("status_id", "12")->where("book_origin","uae")->where("publication_date",$month_start,">=")->where("publication_date",$month_end,"<=")->get();
